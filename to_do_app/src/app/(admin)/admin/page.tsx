...
export default function Dashboard() {
  ...
  // Fetch tasks from the admin endpoint
  ...
  useEffect(() => {
    async function fetchUsers() {
      try {
        ...
        const data = await res.json();
        setUsers(data);
        // Set a default user if not already set and if any users exist
  ...
  const handleOpenAddModal = () => {
    setForm({ 
      ...
      duedate: today, 
      status: 0, 
      userId: form.userId || (users.length > 0 ? users[0].id : "")
    });
    setDateError("");
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (editingTaskId) {
      try {
        const res = await fetch(`/api/task/admin/${editingTaskId}`, {
          method: "PUT",
  ...
  const handleDeleteAll = async () => {
    try {
      const res = await fetch("/api/task/admin/deleteall", { method: "DELETE" });
      if (res.ok) {
        await fetchTasks();
  ...
  return (
    <div className="bg-gradient-to-br from-gray-900 to-purple-900 text-white min-h-screen flex flex-col">
      <header className="backdrop-blur-sm bg-black/30 sticky top-0 z-10 flex items-center justify-between px-8 py-4 border-b border-purple-500/30">
        <h1 className="pl-8 text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-pink-300">Admin Dashboard</h1>
        <div className="flex space-x-3">
          <button
            className="rounded-full border border-red-500 px-6 py-2 font-semibold shadow-lg hover:bg-red-600 hover:border-transparent hover:shadow-red-600/50 focus:outline-none focus:ring-2 focus:ring-red-500 transition-all duration-300 ease-in-out"
                      ...
                      {pendingTasks.map((task) => (
                        ...
                        >
                          ...
                          <div className="flex items-center text-sm text-gray-300 mb-4">
                            ...
                            <span>Assigned to: {users.find(user => user.id === task.user_id)?.username || 'Unknown'}</span>
                          </div>
        ...
        <div className="fixed inset-0 z-20 flex items-center justify-center bg-black bg-opacity-75 backdrop-blur-sm">
          <div className="bg-gray-800 border border-purple-500/30 p-6 rounded-xl shadow-2xl max-w-lg w-full mx-4 transform transition-all">
            ...
            <form className="flex flex-col space-y-4" onSubmit={handleSubmit}>
              ...
              {/* User Dropdown - Admin specific */}
              <div>
                <label htmlFor="user-add" className="block mb-1 font-medium">
                  Assigned To
                </label>
                <select
                  id="user-add"
        ...
        <div className="fixed inset-0 z-20 flex items-center justify-center bg-black bg-opacity-75 backdrop-blur-sm">
          <div className="bg-gray-800 border border-purple-500/30 p-6 rounded-xl shadow-2xl max-w-lg w-full mx-4 transform transition-all">
            ...
            <form className="flex flex-col space-y-4" onSubmit={handleSubmit}>
              ...
              {/* User Dropdown - Admin specific */}
              <div>
                <label htmlFor="user-edit" className="block mb-1 font-medium">
                  Assigned To
                </label>
                <select
                  id="user-edit"
      ...
      {showDeleteAllConfirmation && (
        <div className="fixed inset-0 z-20 flex items-center justify-center bg-black bg-opacity-75 backdrop-blur-sm">
          <div className="bg-gray-800 border border-red-500/30 p-6 rounded-xl shadow-2xl max-w-md w-full mx-4 transform transition-all">
            <h2 className="text-2xl font-bold mb-4 text-red-400">Delete All Tasks</h2>
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button
                type="button"
                className="rounded-lg bg-red-600 px-5 py-2 font-semibold shadow-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 transition"
